...
template <class TileShape_,
          ...
          class KernelSched_  = cutlass::gemm::KernelTmaWarpSpecializedCooperative,
          class EpiSched_     = cutlass::epilogue::TmaWarpSpecializedCooperative,
          int   AuxStagesM_   = 0,
          int   AuxStagesV_   = 0>
struct HopperEVTAdamW {
  ...
  using EpilogueSchedule = EpiSched_;
  ...
  // ── AuxStages: CORRECTNESS-CRITICAL. Default 0. ───────────────────────
  // With AuxStages>=1 the M and V aux nodes are the SAME C++ type
  //   Sm90AuxLoad<Stages, EpilogueTile, bf16, Stride, SmemLayoutAtom, ...>
  // so CUTLASS's SMEM allocator POOLS their staging buffers → V's TMA load
  // stomps M's smem before the W-update reads it. Result: W and m are wrong
  // for step>=2 (step 1 hides it because m=v=0 → corruption × 0). Verified
  // by micro_m4.py: AuxStages=1 gives eW≈1e0, eM≈2e-1 at step=10.
  // AuxStages=0 selects the CUTLASS 3.6+ Sm90AuxLoad/Store<Stages=0>
  // specialisation: direct GMEM↔register, EMPTY SharedStorage{}, zero TMA
  // descriptors → no cross-aux aliasing AND frees all epilogue SMEM back to
  // the mainloop. Both correct and faster. Kept as a template param so the
  // buggy =1 path stays measurable.
  // Separate M/V stage counts. When AuxStagesM_ != AuxStagesV_ the M-chain and
  // V-chain aux nodes become DISTINCT C++ types, so CUTLASS's per-type
  // SharedStorage pooling can no longer alias them — the fix for the M/V
  // stomping bug that lets us keep TMA staging (Stages>=1) AND stay correct.
  static constexpr int AuxStagesM = AuxStagesM_;
  static constexpr int AuxStagesV = AuxStagesV_;

  using AuxLoadM = cutlass::epilogue::fusion::Sm90AuxLoad<
      AuxStagesM, typename EpiDesc::EpilogueTile, ElementAux,
      typename AuxLoadDesc::Stride, typename AuxLoadDesc::SmemLayoutAtom,
      typename AuxLoadDesc::CopyOpS2R, AlignmentAux>;
  using AuxLoadV = cutlass::epilogue::fusion::Sm90AuxLoad<
      AuxStagesV, typename EpiDesc::EpilogueTile, ElementAux,
      typename AuxLoadDesc::Stride, typename AuxLoadDesc::SmemLayoutAtom,
      typename AuxLoadDesc::CopyOpS2R, AlignmentAux>;

  using AuxStoreM = cutlass::epilogue::fusion::Sm90AuxStore<
      AuxStagesM, typename EpiDesc::EpilogueTile, ElementAux, RoundStyle,
      typename AuxStoreDesc::Stride, typename AuxStoreDesc::SmemLayoutAtom,
      typename AuxStoreDesc::CopyOpR2S, AlignmentAux>;
  using AuxStoreV = cutlass::epilogue::fusion::Sm90AuxStore<
      AuxStagesV, typename EpiDesc::EpilogueTile, ElementAux, RoundStyle,
      typename AuxStoreDesc::Stride, typename AuxStoreDesc::SmemLayoutAtom,
      typename AuxStoreDesc::CopyOpR2S, AlignmentAux>;
  ...
  using M_New_Compute = cutlass::epilogue::fusion::Sm90EVT<
      MComputeNode, Sm90AccFetchT, AuxLoadM>;
  using M_New_Stored = cutlass::epilogue::fusion::Sm90EVT<AuxStoreM, M_New_Compute>;
  ...
  using V_New_Compute = cutlass::epilogue::fusion::Sm90EVT<
      VComputeNode, Sm90AccFetchT, AuxLoadV>;
  using V_New_Stored = cutlass::epilogue::fusion::Sm90EVT<AuxStoreV, V_New_Compute>;
...
// Buggy same-type AuxStages: M=V=1 → pooled → aliased (perf-delta reference).
using EVT_AdamW_128x256_a1 = HopperEVTAdamW<
    Shape<_128,_256,_64>, Cluster2x1,
    cutlass::epilogue::collective::EpilogueTileAuto,
    cutlass::gemm::KernelTmaWarpSpecializedCooperative,
    cutlass::epilogue::TmaWarpSpecializedCooperative, /*M=*/1, /*V=*/1>;

// DISTINCT-TYPE aux experiment: M and V use different Stages so their aux
// nodes are different C++ types → CUTLASS can't pool/alias their SMEM, while
// BOTH keep TMA staging (correct AND fast — the goal). Test M=1/V=2, M=2/V=1.
using EVT_AdamW_128x256_m1v2 = HopperEVTAdamW<
    Shape<_128,_256,_64>, Cluster2x1,
    cutlass::epilogue::collective::EpilogueTileAuto,
    cutlass::gemm::KernelTmaWarpSpecializedCooperative,
    cutlass::epilogue::TmaWarpSpecializedCooperative, /*M=*/1, /*V=*/2>;
using EVT_AdamW_128x256_m2v1 = HopperEVTAdamW<
    Shape<_128,_256,_64>, Cluster2x1,
    cutlass::epilogue::collective::EpilogueTileAuto,
    cutlass::gemm::KernelTmaWarpSpecializedCooperative,
    cutlass::epilogue::TmaWarpSpecializedCooperative, /*M=*/2, /*V=*/1>;
// M=0/V=1: M direct-register (can't be stomped), V staged (distinct type).
using EVT_AdamW_128x256_m0v1 = HopperEVTAdamW<
    Shape<_128,_256,_64>, Cluster2x1,
    cutlass::epilogue::collective::EpilogueTileAuto,
    cutlass::gemm::KernelTmaWarpSpecializedCooperative,
    cutlass::epilogue::TmaWarpSpecializedCooperative, /*M=*/0, /*V=*/1>;

// EXPERIMENTAL: Pingpong schedule — 2 math warpgroups on DIFFERENT tiles so
// one tile's AdamW epilogue overlaps the next tile's WGMMA mainloop (the
// overlap the cooperative schedule serializes). tile_m=64 (one warpgroup per
// tile); epilogue uses the plain TmaWarpSpecialized tag (pingpong-paired).
using Cluster1x1 = Shape<_1,_1,_1>;
using EVT_AdamW_pp_64x256 = HopperEVTAdamW<
    Shape<_64,_256,_64>, Cluster1x1,
    cutlass::epilogue::collective::EpilogueTileAuto,
...
void hopper_evt_adamw_pp_64x256(
    torch::Tensor go, torch::Tensor inp,
    torch::Tensor weight, torch::Tensor m, torch::Tensor v,
    float lr, float beta1, float beta2, float eps, float weight_decay,
    float bc1, float bc2)
{
  EVT_AdamW_pp_64x256::launch(go, inp, weight, m, v,
                              lr, beta1, beta2, eps, weight_decay, bc1, bc2);
}

#define AUX_VARIANT(NAME, TYPE)                                               \
  void hopper_evt_adamw_##NAME(                                              \
      torch::Tensor go, torch::Tensor inp,                                   \
      torch::Tensor weight, torch::Tensor m, torch::Tensor v,                \
      float lr, float beta1, float beta2, float eps, float weight_decay,     \
      float bc1, float bc2)                                                  \
  { TYPE::launch(go, inp, weight, m, v, lr, beta1, beta2, eps,               \
                 weight_decay, bc1, bc2); }

AUX_VARIANT(128x256_a1,   EVT_AdamW_128x256_a1)
AUX_VARIANT(128x256_m1v2, EVT_AdamW_128x256_m1v2)
AUX_VARIANT(128x256_m2v1, EVT_AdamW_128x256_m2v1)
AUX_VARIANT(128x256_m0v1, EVT_AdamW_128x256_m0v1)


PYBIND11_MODULE(TORCH_EXTENSION_NAME, mod) {
  mod.def("hopper_evt_adamw_pp_64x256", &hopper_evt_adamw_pp_64x256,
          "Sm90 EVT-fused grad_W + AdamW (64x256x64, PINGPONG schedule)");
  ...
  mod.def("hopper_evt_adamw_128x256_a1", &hopper_evt_adamw_128x256_a1,
          "BUGGY M=V=1 (pooled/aliased) 128x256 — perf-delta measurement only");
  mod.def("hopper_evt_adamw_128x256_m1v2", &hopper_evt_adamw_128x256_m1v2,
          "distinct-type aux M=1 V=2 (correct+staged test)");
  mod.def("hopper_evt_adamw_128x256_m2v1", &hopper_evt_adamw_128x256_m2v1,
          "distinct-type aux M=2 V=1 (correct+staged test)");
  mod.def("hopper_evt_adamw_128x256_m0v1", &hopper_evt_adamw_128x256_m0v1,
          "distinct-type aux M=0 V=1 (correct+staged test)");
}
