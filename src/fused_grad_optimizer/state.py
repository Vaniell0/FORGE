...
class FusedOptimizerState:
    ...
    def pre_step_fp8(self):
        ...
        MIN_AM = 1e-8
        m_new = torch.clamp(self.m_absmax_next, min=MIN_AM) / FP8_E4M3_MAX
        v_new = torch.clamp(self.v_absmax_next, min=MIN_AM) / FP8_E5M2_MAX
        # Delayed-scaling cold-start guard: the stored m_fp8/v_fp8 were quantised
        # with the PREVIOUS step's scale, but this kernel dequants with the current
        # scale (single scale for load+store). At cold start the scale jumps ~1e4x
        # (step-1 amax=0 -> degenerate scale, step-2 real scale), so the old fp8
        # state is dequantised ~1e4x too large -> divergence. Cap the per-step
        # scale INCREASE so load/store mismatch stays bounded (ramps in a few steps;
        # once amax plateaus this is a no-op). NB proper fix = store per-step scale.
        RAMP = 2.0
        if float(self.m_scale) > 0:
            m_new = torch.clamp(m_new, max=self.m_scale * RAMP)
        if float(self.v_scale) > 0:
            v_new = torch.clamp(v_new, max=self.v_scale * RAMP)
        self.m_scale.copy_(m_new)
        self.v_scale.copy_(v_new)
